# Связи между таблица и моделями в Eloquent
...
После определения связей между моделями в Eloquent, мы можем легко получать связанные данные, используя методы-связи.

#### Получение связанных данных "один к одному"

Представим, что у нас есть модели `User` и `Profile`, связанные отношением "один к одному".
...
Представим, что у нас есть модели `Category` и `Post`, связанные отношением "один ко многим".

```php
// Получение всех постов для категории
$category = Category::find(1);
$posts = $category->posts;

// Вывод всех постов для категории
foreach ($posts as $post) {
    echo $post->title;
}

// Получение категории для поста
$post = Post::find(1);
...
Представим, что у нас есть модели `User` и `Role`, связанные отношением "многие ко многим".

```php
// Получение всех ролей для пользователя
$user = User::find(1);
$roles = $user->roles;

// Вывод всех ролей для пользователя
foreach ($roles as $role) {
    echo $role->name;
}

// Получение всех пользователей для роли
$role = Role::find(1);
$users = $role->users;
```
...
Eloquent позволяет использовать **построитель запросов** при получении связанных данных. Это позволяет более гибко настраивать запросы и получать только необходимые данные.
...
Как можно заметить, в данном случае мы используем метод `posts()` для получения всех постов для категории, а затем добавляем дополнительные условия и сортировку.