...
3. Создайте маршрут для главной страницы в файле `routes/web.php`.
   ...
   {
      return view('home'); // Отображает главную страницу
   }
   ```
...
1. Создайте класс-контроллер `TaskController` для обработки запросов, связанных с задачами, и добавьте следующие методы:
   ...
   _На данном этапе эти методы можно оставить пустыми. Вскоре вы начнете работать с представлениями, а в последующих лабораторных работах — с данными._
...
4. Определите правильные имена маршрутов для методов контроллера `TaskController`, например:
   ...
   - `tasks.create` — форма создания задачи.
   - ...
6. Добавьте валидацию параметров маршрута `id` для задач. Убедитесь, что параметр `id` — это положительное целое число. Используйте метод `where`, чтобы ограничить значения для параметра `id`.
5. Вместо ручного создания маршрутов для каждого метода можно использовать **ресурсный контроллер**, который автоматически создаст маршруты для всех **CRUD-операций**:
...
6. Проверьте созданные маршруты с помощью команды `php artisan route:list`.
...
4. Создайте представления для задач со следующими шаблонами в каталоге `resources/views/tasks`:
   ...
   - `create.blade.php` — форма создания задачи;
   - `show.blade.php` — отображение задачи;
   - `edit.blade.php` — форма редактирования задачи.
